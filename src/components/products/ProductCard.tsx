...
const ProductCard = ({ product, index = 0 }: ProductCardProps) => {
  ...
  const handleToggleFavorite = (e: React.MouseEvent) => {
    ...
    toast({ title: fav ? 'Retiré des favoris' : 'Ajouté aux favoris' });
  };
  ...
  return (
    ...
    >
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <div className="absolute bottom-4 right-4 flex gap-2">
              <button
                ...
                className="w-10 h-10 rounded-xl bg-white/90 backdrop-blur-sm flex items-center justify-center hover:bg-white transition-colors focus-ring"
              >
                <Heart className={`w-5 h-5 ${fav ? 'fill-destructive text-destructive' : 'text-foreground'}`} />
              </button>
      ...
      <div className="p-4">
        ...
        <Link to={`/produit/${product.id}`}>
          <h3 className="font-heading font-semibold text-lg text-card-foreground mb-2 line-clamp-1 hover:text-primary transition-colors">{product.name}</h3>
        </Link>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="icon" onClick={handleAddToCart} className="h-10 w-10 rounded-xl hover:bg-primary hover:text-primary-foreground transition-colors focus-ring">
              <ShoppingCart className="w-4 h-4" />
            </Button>
            <Link to={`/produit/${product.id}`}>
              <Button variant="default" size="sm" className="rounded-xl px-4">Voir détails</Button>
            </Link>
          </div>
        </div>
      </div>
