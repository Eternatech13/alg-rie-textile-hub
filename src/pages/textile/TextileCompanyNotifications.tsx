...
import { textileNotifications } from "@/data/mockTextileCompanyData";
...
export default function TextileCompanyNotifications() {
  const unreadCount = textileNotifications.filter((n) => !n.read).length;
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="pt-6 space-y-2">
          {textileNotifications.map((notif) => {
            const Icon = iconMap[notif.type] || ShoppingCart;
            return (
              <div key={notif.id} className={`flex items-start gap-4 p-4 rounded-lg transition-colors ${notif.read ? "bg-background" : "bg-primary/5 border border-primary/10"}`}>
