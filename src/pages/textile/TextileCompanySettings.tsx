...
export default function TextileCompanySettings() {
  ...
  const [notifications, setNotifications] = useState({
    orders: true, conventions: true, stock: true, reports: true,
  });
  ...
  const toggleNotification = (key: keyof typeof notifications) => {
    setNotifications((prev) => {
      ...
      toast({ title: updated[key] ? "Notification activée" : "Notification désactivée" });
      return updated;
    });
  };
  ...
  const toggleVisibility = (key: keyof typeof visibility) => {
    setVisibility((prev) => {
      const updated = { ...prev, [key]: !prev[key] };
      toast({ title: updated[key] ? "Activé" : "Désactivé" });
      return updated;
    });
  };
  ...
  const handleDeleteAccount = () => {
    setDeleteOpen(false);
    toast({ title: "Demande envoyée", description: "Votre demande de suppression de compte a été transmise à l'administration." });
  };
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Switch checked={visibility.publicProfile} onCheckedChange={() => toggleVisibility("publicProfile")} />
          </div>
          <div className="flex items-center justify-between">
            ...
            <Switch checked={visibility.showCapacity} onCheckedChange={() => toggleVisibility("showCapacity")} />
          </div>
        </CardContent>
      </Card>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Cette action est irréversible. Toutes vos données, produits et commandes seront perdues.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter><AlertDialogCancel>Annuler</AlertDialogCancel><AlertDialogAction onClick={handleDeleteAccount} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Supprimer définitivement</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
  );
